...
namespace globals {
  namespace components {
    ...
    ModbusUART::ModbusUART(const py::object *parent_actor,
        ...
        : ModbusUARTBase(parent_actor, actor_spec, type_spec, name, type_name, args, application_name,
                         actor_name) {
        PID = getpid();
        ...
        if (ibaudrate != nullptr) {
            if(!ibaudrate->getValueAsInt(&(portConfig.baudrate))) {
                component_logger()->error("{}: Invalid Baud Rate argument - {}, should be integer value", PID, portConfig.baudrate);
            }
        } else {
            // Declare a default value
        ...
        if (ibytesize != nullptr) {
            if(!ibytesize->getValueAsInt(&(portConfig.bytesize))) {
                component_logger()->error("{}: Invalid Byte Size argument - {}, should be integer value", PID, portConfig.bytesize);
            }
        } else {
            // Declare a default value
        ...
        if (istopbits != nullptr) {
            if(!istopbits->getValueAsInt(&(portConfig.stopbits))) {
                component_logger()->error("{}: Invalid Stop Bits argument - {}, should be integer value", PID, portConfig.stopbits);
            }
        } else {
            // Declare a default value
        ...
        component_logger()->info("{}: Modbus UART settings {} @{}:{} {}{}{}", PID, portSlaveAddress, portConfig.portname,
    ...
    void ModbusUART::OnModbusRepPort(const modbusuart::messages::CommandFormat::Reader &message,
      ...
      if (isModbusReady()) {
          ...
          component_logger()->warn("{}: Modbus is not ready for commands", PID);
      }
    ...
    void ModbusUART::OnClock(riaps::ports::PortBase *port) {
      ...
      if (!isModbusReady() && (ctx != NULL)) {
          ...
          if (startRTUModbus(portSerialMode)) {
              component_logger()->debug("{}: Started Modbus: port={}, slaveAddress={}", PID, portConfig.portname, portSlaveAddress);
          }
      }
      ...
      if (timerPort != nullptr) {
          ...
          if (t != nullptr) {
              t->stop();
              component_logger()->info("{}: Stopped Clock port", PID);
          }
      }
    }
    ...
    int ModbusUART::sendModbusCommand(const modbusuart::messages::CommandFormat::Reader &message) {
        ...
        switch (message.getCommandType()) {
            case modbusuart::messages::ModbusCommands::READ_COIL_BITS:
                ...
                component_logger()->debug("{}: Sent command {}, register={}, numOfDecimals={}", PID, "READ_COILBITS",
                               regAddress, numRegs);
                break;
            case modbusuart::messages::ModbusCommands::READ_INPUT_BITS:
                value = readInputBits(regAddress, numRegs);
                component_logger()->debug("{}: Sent command {}, register={}, numOfDecimals={}", PID, "READ_INPUTBITS",
                               regAddress, numRegs);
                break;
            case modbusuart::messages::ModbusCommands::READ_INPUT_REGS:
                value = readInputRegs(regAddress, numRegs);
                component_logger()->debug("{}: Sent command {}, register={}, numOfRegs={}", PID, "READ_INPUTREGS",
                               regAddress, numRegs);
                break;
            case modbusuart::messages::ModbusCommands::READ_HOLDING_REGS:
                value = readHoldingRegs(regAddress, numRegs);
                component_logger()->debug("{}: Sent command {}, register={}, numOfRegs={}", PID, "READ_HOLDINGREGS",
                               regAddress, numRegs);
                break;
            case modbusuart::messages::ModbusCommands::WRITE_COIL_BIT:
                ...
                component_logger()->debug("{}: Sent command {}, register={}, value={}", PID, "WRITE_COILBIT", regAddress,
                               writeValue);
                break;
            case modbusuart::messages::ModbusCommands::WRITE_HOLDING_REG:
                writeValue = (int) message.getValues()[0];
                value = writeHoldingReg(regAddress, writeValue);
                component_logger()->debug("{}: Sent command {}, register={}, value={}", PID, "WRITE_HOLDINGREG", regAddress,
                               writeValue);
                break;
            case modbusuart::messages::ModbusCommands::WRITE_COIL_BITS:
                ...
                component_logger()->debug("{}: Sent command {}, register={}, numberOfDecimals={}", PID, "WRITE_COILBITS",
                               regAddress, numRegs);
                component_logger()->debug("{}: Values:{}", PID, valueLogMsg);
                break;
            case modbusuart::messages::ModbusCommands::WRITE_MULTI_HOLDING_REGS:
                ...
                component_logger()->debug("{}: Sent command {}, register={}, numOfRegs={}", PID, "WRITEMULTI_HOLDINGREGS",
                               regAddress, numRegs);
                component_logger()->debug("{}: Values:{}", PID, valueLogMsg);
                break;
            case modbusuart::messages::ModbusCommands::WRITE_READ_HOLDING_REGS:
                ...
                break;
            default:
                component_logger()->warn("{}: Invalid Command - {}", PID, (int16_t) message.getCommandType());
        }
    ...
    */
    ...
    bool ModbusUART::startRTUModbus(int serialMode) {
        ...
        if (modbus_connect(ctx) == -1) {
            ...
            portOpen = true;
            component_logger()->info("Modbus port open. Port={}, Baud={}, {}{}{}", portConfig.portname, portConfig.baudrate,
...
create_component_py(const py::object *parent_actor,
                    ...
                    const std::string &application_name,
                    const std::string &actor_name) {
    auto ptr = new globals::components::ModbusUART(parent_actor, actor_spec, type_spec, name, type_name, args,
                                                           application_name,
                                                           actor_name);
    return std::move(std::unique_ptr<globals::components::ModbusUART>(ptr));
}

PYBIND11_MODULE(libmodbusuart, m) {
    py::class_<globals::components::ModbusUART> testClass(m, "ModbusUART");
    ...
    testClass.def("setup"                 , &globals::components::ModbusUART::Setup);
    testClass.def("activate"              , &globals::components::ModbusUART::Activate);
    testClass.def("terminate"             , &globals::components::ModbusUART::Terminate);
    testClass.def("handlePortUpdate"      , &globals::components::ModbusUART::HandlePortUpdate);
    testClass.def("handleCPULimit"        , &globals::components::ModbusUART::HandleCPULimit);
    testClass.def("handleMemLimit"        , &globals::components::ModbusUART::HandleMemLimit);
    testClass.def("handleSpcLimit"        , &globals::components::ModbusUART::HandleSpcLimit);
    testClass.def("handleNetLimit"        , &globals::components::ModbusUART::HandleNetLimit);
    testClass.def("handleNICStateChange"  , &globals::components::ModbusUART::HandleNICStateChange);
    testClass.def("handlePeerStateChange" , &globals::components::ModbusUART::HandlePeerStateChange);
