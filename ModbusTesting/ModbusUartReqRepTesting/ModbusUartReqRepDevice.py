...
#            pydevd.settrace(host='192.168.1.102',port=5678)
    ...
    def on_modbusStatusRepPort(self):
        ...
        try:
            ...
            self.logger.info("on_modbusStatusRepPort:receive exception = %d" % e.errno)
            if e.errno in (PortError.EAGAIN,PortError.EPROTO):
                self.logger.info("on_modbusStatusRepPort: port error received")
        ...
        if self.statusRequestPending == 1:
            try:
                ...
                self.logger.info("on_modbusStatusRepPort:send exception = %d" % e.errno)
                if e.errno in (PortError.EAGAIN,PortError.EPROTO):
                    self.logger.info("on_modbusStatusRepPort: port error received")
    ...
    def on_modbusCommandRepPort(self):
        ...
        try:
            ...
            self.logger.info("Request for Modbus device command received")
            #if debugMode:
            ...
            self.logger.info("on_modbusCommandRepPort:receive exception = %d" % e.errno)
            if e.errno in (PortError.EAGAIN,PortError.EPROTO):
                self.logger.info("on_modbusCommandRepPort: port error received")
        ...
        if self.commandRequestPending == 1:
            ...
            try:
                ...
                self.commandRequestPending -= 1
                self.logger.info("Response for Modbus device command sent")
            except PortError as e:
                self.logger.info("on_modbusCommandRepPort:send exception = %d" % e.errno)
                if e.errno in (PortError.EAGAIN,PortError.EPROTO):
                    self.logger.info("on_modbusCommandRepPort: port error received")
    ...
    def sendModbusCommand(self):
        ...
        #if debugMode:
        #    t1 = time.perf_counter()
        #    self.logger.info("sendModbusCommand()[%s]: Modbus library command complete at %f, time to interact with Modbus library is %f ms" % (str(self.pid),t1,(t1-t0)*1000))
